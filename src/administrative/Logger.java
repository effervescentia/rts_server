	...
	public boolean initialize(List<String> fileNames, List<String> comPorts){
		...
		if (fileNames.size() == comPorts.size()){
			for(int i = 0; i < fileNames.size(); i++){
				...
				try{
					CommPortIdentifier portIdentifier = CommPortIdentifier.getPortIdentifier(comPorts.get(i));
				...
				channelList.add(new outputChannel(fileNames.get(i), outputFile, serialchannel));
			}
			return true;
		}else
			return false;
